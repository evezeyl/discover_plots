...
selected_data <- 
  ...
  select(Country, Source.Group, 
         ...
         stx.other.method,
         Serotype.O, Serotype.H, SerotypeOH, eae, eae.y,
         AMR.genes,
         Source, 
...
write.table(selected_data %>% 
              select( -n ) %>%
              my_summary_fun(),
            here("results", "2025-02-07_summary_table_col.tsv"),
            sep = "\t", 
...
```{r}
write.table(stx_all,
            here("results", "2025-02-07_stx_all.tsv"),
            sep = "\t", 
            col.names = T, row.names = F,
            quote = F)
```
...
parallel_data1 <- 
  data %>%
  select(Source, eae, stx := stx.subtype, SerotypeOH, n) %>%
...
length(sero_levels) #10 + Other should be ok
```
...
# NA count is O
source_levels
source_levels <- paste(source_levels, c(count_origin, 0), sep = "\n")
source_levels
...
eae_levels
eae_levels <- paste(eae_levels, c(count_eae,0), sep = "\n")
eae_levels
...
count_stx <- 
  ...
  pull(sum_stx)
count_stx
# here there is NA that is counted
stx_levels
stx_levels <- paste(stx_levels, count_stx, sep = "\n")
stx_levels
...
# Here there is no NA 
sero_levels
sero_levels <- paste(sero_levels, c(count_OH,0), sep = "\n")
sero_levels
```
...
```{r}
levels(interaction(parallel_data3$Source, parallel_data3$stx))

parallel_data_long3 <- 
  parallel_data3 %>%
  # this did not work 
  # arrange(
  #   factor(interaction(Source, stx), 
  #          levels = c("Human.stx1\n(850)", "Human.stx2\n(1755)","Human.stx1;\nstx2\n(791)", 
  #                     "Non Human.stx1\n(850)", "Non Human.stx2\n(1755)", "Non Human.stx1;\nstx2\n(791)",
  #                     "Human.NA\n(22)",  "Non Human.NA\n(22)")
  #          ))%>%
...
head(parallel_data_long3)  
# so alluvium is link the individual data - what is what 
# so I need to organize those proprely  
```
...
parallel_data_long3 %>%
  ...
  ggplot(.,
         aes(x = Types, y = `%`,
             ...
             alluvium = alluvium,
             label = stratum)) +
  geom_alluvium(aes(fill = Source), aes.bind = "alluvia") +
...
global_parallel_plot3 <- 
  ...
  # need to be second
    geom_fit_text(
        stat = "stratum", min.size = 4) +
  geom_flow(aes(fill = Source)) + 
...
```{r}
scaling <- 3
ggsave(filename = here::here("results", "global_parallel_plot3.png"),
...
low_cov <- 
  data %>%
  select(Avg.Scaffold.coverage, stx.subtype, Serotype.O, Serotype.H,  MLST, Number.cgMLST.loci) %>%
  arrange(Avg.Scaffold.coverage) %>%
  mutate(stx_NT = ifelse(is.na(stx.subtype), "Non Typeable", "Typed"),
...
plot_cov1 <- low_cov_long %>%
  ggplot(aes(x = Typeable,  Avg.Scaffold.coverage, fill = `Typing Method`)) +
  geom_boxplot() +
  theme_bw()
plot_cov1 
...
scaling <- 3
ggsave(filename = here::here("results", "Country_Origin_plot.png"),
...
human_data <- 
  ...
  select(Disease := "Disease..only.human.isolates." , Country) 
...
human_data2 <- 
  ...
  mutate(Country = factor(Country, levels = country_order2, ordered = T),
         Disease = factor(Disease, levels = disease_order, ordered = T)) %>%
...
scaling <- 3
ggsave(filename = here::here("results", "disease_country_plot.png"),
...
data %>%
  ...
  select("Accession.Number", "Isolate.ID", 
         ...
         "Age.group..only.human.isolates.", "Disease..only.human.isolates.", "Import..only.non.human.isolates.", 
         "Sample.Type", 
         "Avg.Scaffold.coverage", "Burst.size", 
         "Number.cgMLST.loci", "MLST", 
         "stx.subtype", "stx.subtype_detail", "stx.other.method", 
         "Serotype.O", "Serotype.H", "SerotypeOH", 
         "AMR.genes", 
         "Species", "Gene", "Count.contaminants", 
         "eae", 
         "asta", "agg3a", "cif", "espj", "toxb", "lpfa", "agg3d", "katp", 
         "espa", "ltca", "cnf1", "espc", "espb", "aata", "stx1a", "tir", 
         "cdtb", "iss", "espf", "tsh", "espp", "sfas", "k88ab", "bfpa", 
         "efa1", "capu", "eila", "pic", "cma", "vat", "siga", "aar", "stx2a", 
         "ehxa", "mchb", "senb", "iha", "stx2b", "fim41a", "etpd", "mcma", 
         "nleb", "nlea", "sat", "feda", "iron", "suba", "pet", "pera", 
         "air", "agg3c", "aaic", "eae.y", "epea", "gad", "sepa", "fedf", 
         "aap", "orf4", "aggr", "f17a", "nlec", "orf3", "cba", "stb", 
         "mchc", "sta1", "agg3b", "mchf", "stx1b", "f17g", "celb", "espi", 
         "irea") %>%
    write.table(., 
                here("results", "2025-02-07_reordered_data.tsv"),
            sep = "\t", 
            col.names = T, row.names = F,
            quote = F)
```
